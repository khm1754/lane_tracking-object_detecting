...
while success:
    ...
    while y>0:
        ...
        cv2.circle(msk, (mid_base, y), 5, (255,255,255), -1)
        aa = msk.copy()
        if aa.shape[-1] != 3:
            aa = cv2.cvtColor(aa, cv2.COLOR_GRAY2BGR) 
        rematrix = np.linalg.inv(matrix)
        retransformed = cv2.warpPerspective(aa, rematrix,(640,480))
        cap = cv2.addWeighted(frame, 1, retransformed, 1, 0)

        if mid_base == midpoint:
            print("go")
        elif mid_base > midpoint:
            print("right")
        else:
            print("left")
